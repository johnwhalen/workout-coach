...
async function testChatHistory() {
  ...
  try {
    ...
    const testMessage = JSON.stringify({ text: "__TEST_MESSAGE__", isUser: true, timestamp: new Date().toISOString() });
...
async function testWorkoutOperations() {
  ...
  try {
    ...
    recordTest(`Found ${totalWorkouts} workouts across ${routines.length} routines`, totalWorkouts >= 0);
...
async function testStreamingChat() {
  ...
  const testPrompts = [
    ...
    { name: "Workout logging without routine", prompt: "Log 3 sets of 10 reps bench press at 135 lbs" },
  ...
  for (const test of testPrompts) {
    try {
      ...
      if (response.ok && response.body) {
        ...
        recordTest(`${test.name} - Stream response`, !hasError && hasComplete, hasError ? "Error in stream" : "");
      } else {
...
async function testEdgeCases() {
  ...
  try {
    ...
    recordTest("Empty prompt handling", response.ok || response.status === 400, `Status: ${response.status}`);
  } catch (error) {
...
async function testDataIntegrity() {
  ...
  try {
    ...
    recordTest(`No orphaned workouts (found ${orphanedWorkouts.length})`, orphanedWorkouts.length === 0);
