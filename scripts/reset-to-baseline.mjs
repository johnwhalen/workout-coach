...
async function resetToBaseline() {
  ...
  const recentSets = allSets.filter(s =>
    s.date.toISOString().split('T')[0] === mostRecentDate
  );
  ...
  const olderSets = allSets.filter(s =>
    s.date.toISOString().split('T')[0] !== mostRecentDate
  );
  ...
  console.log("\nTotal: " + baselineSets.length + " sets across " + Object.keys(byExercise).length + " exercises");
