...
export async function GET(req: NextRequest) {
    try {
        ...
        const workoutsByDate = workouts.reduce((acc: Record<string, WorkoutSummary[]>, workout: typeof workouts[number]) => {
            ...
            acc[dateKey].push({
                ...
                total_reps: workout.Set.reduce((sum: number, set: typeof workout.Set[number]) => sum + set.set_reps, 0),
                total_weight: workout.Set.reduce((sum: number, set: typeof workout.Set[number]) => sum + set.set_weight * set.set_reps, 0),
            });
