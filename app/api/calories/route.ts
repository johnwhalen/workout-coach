...
export async function POST(req: NextRequest) {
  try {
    ...
    const calorieData = workouts.map((workout: (typeof workouts)[number]) => {
      const setsCalories = workout.Set.reduce(
        (total: number, set: (typeof workout.Set)[number]) => {
    ...
    const dailyTotals = calorieData.reduce(
      (
        acc: Record<string, { date: string; totalCalories: number; workouts: typeof calorieData }>,
        workout: (typeof calorieData)[number]
      ) => {
    ...
    const totalCaloriesBurned = calorieData.reduce(
      (total: number, workout: (typeof calorieData)[number]) => total + workout.totalCalories,
      0
    );
